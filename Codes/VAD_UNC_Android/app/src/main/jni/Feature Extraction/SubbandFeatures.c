...
SubbandFeatures* initSubbandFeatures(int nFFT, int stepsize, int decisionBufferLength)
{
    ...
    {
        newSubbandFeatures->framePeriodicityBuffer[i]=calloc(bufferLength,sizeof(float*));
    }
    newSubbandFeatures->frameEntropyBuffer = (float**)malloc(beCalcBands*sizeof(float*));
    for (i=0;i<beCalcBands;i++)
    {
        newSubbandFeatures->frameEntropyBuffer[i]=calloc(bufferLength,sizeof(float*));
    }
...
//					norm_temp = norm; //Save the norm for the next frame
            ...
            // Nasim: Add the computed cbn to feature matrix
            //sbf->FeatureMatrix[count][b] =  cbn_max;
        ...
        if (BE_BAND_CALC[b]==1) // only calculate band features we plan to use
        {
            ...
            //Nasim: add the computed entropybn to feature matrix
            //sbf->FeatureMatrix[count][b + NBANDS] = entropybn;
